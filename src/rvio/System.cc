...
System::System(const std::string& strSettingsFile) {
  ...
  mnImuRate = fsSettings["IMU.dps"]; // IMU的频率

  msigmaGyroNoise = fsSettings["IMU.sigma_g"]; // 陀螺仪噪声
  msigmaGyroBias = fsSettings["IMU.sigma_wg"]; // 陀螺仪随机游走
  msigmaAccelNoise = fsSettings["IMU.sigma_a"]; // 加速度计噪声
  msigmaAccelBias = fsSettings["IMU.sigma_wa"]; // 加速度计随机游走

  mnGravity = fsSettings["IMU.nG"]; // 重力加速度的大小

  mnCamTimeOffset = fsSettings["Camera.nTimeOffset"]; // 相机-IMU时间偏移

  const int nMaxTrackingLength = fsSettings["Tracker.nMaxTrackingLength"];
  mnSlidingWindowSize = nMaxTrackingLength - 1; // 滑动窗口大小
  ...
  mbEnableAlignment = bEnableAlignment; // 是否启用重力对齐

  const int bRecordOutputs = fsSettings["INI.RecordOutputs"];
  mbRecordOutputs = bRecordOutputs; // 是否记录输出
...
void System::MonoVIO() {
  ...
  if (nCloneStates > mnMinCloneStates) {
    mpUpdater->update(mpPreIntegrator->xk1k, mpPreIntegrator->Pk1k,
                      ...
                      mpTracker->mvlFeatMeasForUpdate);
  ...
  if (nImageCountAfterInit > 1) {
    if (nCloneStates < mnSlidingWindowSize) {
      ...
      // Pkk
      Eigen::MatrixXd J(24 + 6 * (mnSlidingWindowSize + 1),
                        24 + 6 * mnSlidingWindowSize);
      J.setZero();
      J.block(0, 0, 24 + 6 * mnSlidingWindowSize, 24 + 6 * mnSlidingWindowSize)
          .setIdentity();
      ...
      Pkk.block(0, 24, 24, 6 * mnSlidingWindowSize) =
          tempP.block(0, 24 + 6, 24, 6 * mnSlidingWindowSize);
      Pkk.block(24, 0, 6 * mnSlidingWindowSize, 24) =
          tempP.block(24 + 6, 0, 6 * mnSlidingWindowSize, 24);
      Pkk.block(24, 24, 6 * mnSlidingWindowSize, 6 * mnSlidingWindowSize) =
          tempP.block(24 + 6, 24 + 6, 6 * mnSlidingWindowSize,
                      6 * mnSlidingWindowSize);
    }
  }
  ...
  Pkk.block(0, 24, 24, 6 * nCloneStates) =
      Vk * Pkk.block(0, 24, 24, 6 * nCloneStates);
