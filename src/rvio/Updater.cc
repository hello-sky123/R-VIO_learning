...
std_msgs::ColorRGBA colorLandmark; // ROS标准消息中的颜色类型
geometry_msgs::Vector3 scaleLandmark;

Updater::Updater(const cv::FileStorage& fsSettings) {
  mnCamRate = fsSettings["Camera.fps"]; // 相机帧率
  ...
  mFeatPub =
      mUpdaterNode.advertise<visualization_msgs::Marker>("/rvio/landmarks", 1);
  mnPubRate = fsSettings["Landmark.nPubRate"]; // 发布频率 5Hz

  scaleLandmark.x = fsSettings["Landmark.nScale"]; // size of point
...
void Updater::update(
  ...
  for (int featIdx = 0; featIdx < nFeat; ++featIdx) {
    ...
    for (int i = 1; i < nTrackPhases; ++i) {
      Eigen::Vector4d qI =
          QuatMul(mRelPoses.block(7 * i, 0, 4, 1),
                  mRelPosesToFirst.block(7 * (i - 1), 0, 4, 1));
      Eigen::Vector3d tI = QuatToRot(mRelPoses.block(7 * i, 0, 4, 1)) *
                           (mRelPosesToFirst.block(7 * (i - 1) + 4, 0, 3, 1) -
                            mRelPoses.block(7 * i + 4, 0, 3, 1));
    ...
    for (int nIter = 0; nIter < maxIter; ++nIter) {
      ...
      for (int i = 0; i < nTrackPhases; ++i, ++lit) {
        ...
        Hproj << 1 / h(2), 0, -h(0) / pow(h(2), 2), 0, 1 / h(2),
            -h(1) / pow(h(2), 2);
      ...
      if (cost <= lastCost) {
        ...
        if (fabs(lastCost - cost) < 1e-6 && dpfinv(2) < 1e-6) break;
    ...
    if (fabs(phi) > .5 * 3.14 || fabs(psi) > .5 * 3.14 || std::isinf(rho) ||
        rho < 0) {
    ...
    for (int i = 1; i < nTrackLength; ++i, ++lit) {
      Eigen::Matrix3d R =
          QuatToRot(mRelPosesToFirst.block(7 * (i - 1), 0, 4, 1));

      Eigen::Matrix3d Rc =
          QuatToRot(mCamRelPosesToFirst.block(7 * (i - 1), 0, 4, 1));
      ...
      Eigen::Matrix3d R0T =
          QuatToRot(mRelPosesToFirst.block(0, 0, 4, 1)).transpose();
      ...
      for (int j = 1; j < i; ++j) {
        Eigen::Matrix3d R1T =
            QuatToRot(mRelPosesToFirst.block(7 * j, 0, 4, 1)).transpose();
        Eigen::Vector3d t1 = mRelPosesToFirst.block(7 * j + 4, 0, 3, 1);
        Eigen::Matrix3d R2T =
            QuatToRot(mRelPosesToFirst.block(7 * (j - 1), 0, 4, 1)).transpose();

        Eigen::Matrix3d dpx =
            SkewSymm(mRic * epfinv + rho * mtic + rho * R1T * t1);
        subH << dpx * R1T, -rho * R2T;

        tempHx.block(nStartRow, nStartCol + 6 * j, 2, 6) =
            Hproj * mRci * R * subH;
      }
    ...
    for (int n = 0; n < N; ++n) {
      for (int m = M - 1; m > n; m--) {
        ...
        // G'*Hf
        (tempHf.block(m - 1, n, 2, N - n))
            .applyOnTheLeft(0, 1, tempHf_GR.adjoint());

        // G'*Hx
        (tempHx.block(m - 1, 0, 2, tempHx.cols()))
            .applyOnTheLeft(0, 1, tempHf_GR.adjoint());
    ...
    // Note: this tests the dissimilarity between the measurement and the
    // estimate,
    ...
    tempS = tempHx_ * Pk1k.block(24, 24, 6 * nCloneStates, 6 * nCloneStates) *
            (tempHx_.transpose());
    tempS.diagonal() += tempR;
    tempS = .5 * (tempS + tempS.transpose());

    double nMahalanobisDist =
        (tempr_.transpose() * (tempS.colPivHouseholderQr().solve(tempr_)))
            .norm();
  ...
  if (nGoodFeatCount > 2) {
    ...
    if (Ho.rows() > Ho.cols() - 24) {
      ...
      for (int n = 0; n < N; ++n) {
        for (int m = M - 1; m > n; m--) {
          ...
          //       equivalent to applying G' to the entire row [0:N-1].
          // G'*H
          (tempHw.block(m - 1, n, 2, N - n))
              .applyOnTheLeft(0, 1, tempHw_GR.adjoint());
    ...
    xk1k1.block(14, 0, 12, 1) =
        dx.block(12, 0, 12, 1) + xk1k.block(14, 0, 12, 1);
    ...
    for (int poseIdx = 0; poseIdx < nCloneStates; ++poseIdx) {
      ...
      xk1k1.block(26 + 7 * poseIdx, 0, 4, 1) =
          QuatMul(dqc, xk1k.block(26 + 7 * poseIdx, 0, 4, 1));
      xk1k1.block(26 + 7 * poseIdx + 4, 0, 3, 1) =
          dx.block(24 + 6 * poseIdx + 3, 0, 3, 1) +
          xk1k.block(26 + 7 * poseIdx + 4, 0, 3, 1);
    }
