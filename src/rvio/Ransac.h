 ...
 *
 * @do @p nIterations (min. 16) trials to find the best hypothesis (of essential
 * matrix).
...
class Ransac {
 public:
   ...
   * @use @p R to rotate the points in frame 1 to frame 0 having the same
   * orientation as frame 2.
   * @compute essential matrix by solving a problem with unknow translation
   * only, (p2^T)[tx]p0=0.
   */
   ...
   * @the original inlier flag vector @p vInlierFlag is obtained from the
   * tracking, i.e. @p status of function cv::calcOpticalFlowPyrLK().
   * @the output is the refined flag vector @p vInlierFlag.
   */
  int FindInliers(const Eigen::MatrixXd& Points1,
                  const Eigen::MatrixXd& Points2, std::list<ImuData*>& lImuData,
